...
def refresh_public_sales(house_name: str = Query(..., min_length=2), db: Session = Depends(get_db)):
...
def export_csv(house_name: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    return StreamingResponse(iter([payload]), media_type='text/csv', headers={'Content-Disposition': 'attachment; filename="ventes_lots.csv"'})


@app.get('/api/export/xlsx')
def export_xlsx(house_name: Optional[str] = None, db: Session = Depends(get_db)):
    payload = build_xlsx_bytes(db, house_name)
    return StreamingResponse(iter([payload]), media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', headers={'Content-Disposition': 'attachment; filename="ventes_lots.xlsx"'})
